...
class SystemController extends Controller
{
    //public function __construct() { //  __construct クラスを追加
        //$this->middleware('auth:system');; // ログイン者のみ下記メソッドを実行可能に
    //}
    ...
    public function userIndex(Request $request)
    {
        ...
        return view('system.userList', ['users' => $users]);
    }
    ...
    public function search(Request $request)
    {
        ...
        if(!empty($keyword)) {
            ...
            return view('system.search', $param);
    ...
    public function userSearch(Request $request)
    {
        ...
        if(!empty($keyword)) {
            ...
            return view('system.userSearch', $param);
        } elseif(empty($keyword)) {
            $count = 0;
            $keyword = '';
            $err = 'キーワードが入力されていません。';
    ...
    {
        $stock = Stock::find($id);
        return view('system.show', ['stock' => $stock]);
    }
    ...
    public function delCheck(Request $request,$id)
    {
        ...
        return view('system.delCheck', ['stock' => $stock]);
    }
    //在庫削除DB反映
    ...
    {
        $user = User::find($id);
        return view('system.userDelCheck', ['user' => $user]);
    }
    //アカウント削除DB反映
